...
check_device = (id) => {
  return new Promise(async (resolve, reject) => {
    try {
      firebase
        .get(firebase.ref(database, "SOIL/" + id))
        .then((snapshot) => {
          if (snapshot.exists()) {
...
add_device = (data) => {
  return new Promise(async (resolve, reject) => {
    try {
      firebase
        .set(firebase.ref(database, "SOIL/" + data[0]), {
          long: data[1],
          lat: data[2],
...
function add_history(data) {
  ...
  firebase
    .get(firebase.ref(database, "HISTORY/SOIL/" + data[0]))
    .then((snapshot) => {
      if (snapshot.exists()) {
...
function add_history_aws(data) {
  ...
  firebase
    .get(firebase.ref(database, "HISTORY/WEATHER/" + data[0]))
    .then((snapshot) => {
      if (snapshot.exists()) {
        const existingData = snapshot.val().data;
